...
## Enforcement — five layers, all now implemented

| # | Layer | Implementation | Bypass |
|---|---|---|---|
| 1 | Impossible | `.claude/settings.json` permissions + per-agent `tools:` | cannot |
| 2 | Blocked | `.claude/hooks/gate-b.py` (PreToolUse) | edit local config |
