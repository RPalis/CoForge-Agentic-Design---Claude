...
index = {
    # NO generated_at. CI regenerates this file and fails on a non-empty git diff,
    # so it must be a pure function of its inputs. A today() stamp made the check
    # fail on every day after the last commit — drift or no drift — and the fix it
    # trains is to commit a meaningless one-line diff daily, which teaches everyone
    # to read a red pipeline as noise. When the file was generated is git's to
    # answer: git log -1 --format=%cd -- .ai/index.json
